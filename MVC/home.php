...
require_once('MVC/servicio/estructura.php');
 ...
 if (isset($_POST['consultaAnyo'])) {
 	if ($_POST["exceso"] === 'S') new Excepcion("Límite de 3 agrupadores alcanzado",1);	
 ...
 if (isset($_POST['accion'])) {
  switch ($_POST['accion']) {
   ...
   case 'GuardarDescripcion':
   	...
   	break;
   case 'procesaArchivo':
    if ($_FILES['userfile']['name'] == "") new Excepcion('No se ha seleccionado ningún archivo', 1);
    if ($controlError->hayError() == 0) {
     if ($_FILES['userfile']['size'] == 0) new Excepcion('El tamaño del archivo es 0 (cero)', 1);
    }
    if ($controlError->hayError() == 0) {
