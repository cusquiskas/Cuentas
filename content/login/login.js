...
var login = class {
    ...
    validacion (s,d,e) {
        console.log('login.js -> validacion');
        if (s) {
            sessionStorage.setItem("user", d.root.Detalle.id);
            sessionStorage.setItem("nombre", d.root.Detalle.nombre);
            Moduls.getHeader().load({ url: 'content/header.html', script: true });
        } else {
            validaErroresCBK(d.root);
        }
    }
    
    submita(s,d,e) { 
        console.log('login.js -> submita');
        if (s) {
            let f = e.form, v = e.form.modul.Forms["valida"];
            v.set({password:MD5(f.parametros.email.value+session+MD5(f.parametros.password.value))});
            v.executeForm();
        } else {
            validaErroresCBK(d);
        }
    ...
    addEventos(modulo) {
        $("input[name=password]").keypress(function(event){
            if (event.keyCode==13) {
                let form = Moduls.getBody().Forms['fake'];
